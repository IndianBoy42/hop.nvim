...
local function create_hints_for_line(i, hint_list, hint_mode, context, direction_mode, lines)
	local line_hints = M.mark_hints_line(
		hint_mode,
...
function M.create_hint_list_by_scanning_lines(hint_mode, opts)
	...
	if opts.direction == constants.HintDirection.AFTER_CURSOR then
		...
		create_hints_for_line(1, hint_list, hint_mode, context, {
			cursor_col = context.cursor_pos[2],
			direction = opts.direction,
		}, lines)

		for i = 2, #lines do
			create_hints_for_line(i, hint_list, hint_mode, context, nil, lines)
		end
	elseif opts.direction == constants.HintDirection.BEFORE_CURSOR then
		...
		create_hints_for_line(#lines, hint_list, hint_mode, context, {
			cursor_col = context.cursor_pos[2],
			direction = opts.direction,
		}, lines)
	else
